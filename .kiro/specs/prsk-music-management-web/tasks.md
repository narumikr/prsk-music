...
- [ ] 7. 楽曲管理機能の実装
  - [x] 7.1 MusicTable.vueのスケルトン作成
    - propsとemitsの型定義のみ
    - テンプレートは空のdiv
    - _Requirements: 要件1_
  
  - [x] 7.2 MusicTableのUnit Test作成
    - 楽曲一覧ページアクセス時のテーブル表示
    - YouTubeリンククリック時のモーダル表示
    - テストは失敗する状態（Red）
    - _Requirements: 要件1_
  
  - [x] 7.3 MusicTableのProperty Test作成
  ...
  - [x] 7.5 MusicFormModal.vueのスケルトン作成
    - propsとemitsの型定義のみ
    - テンプレートは空のdiv
    - _Requirements: 要件2, 要件3, 要件7, 要件8_
  
  - [x] 7.6 MusicFormModalのUnit Test作成
  ...
  - [x] 7.7 MusicFormModalのProperty Test作成
  ...
  - [x] 7.9 MusicListPage.vueのスケルトン作成
    - テンプレートは空のdiv
    - useMusicList、useArtistList、useNotificationの呼び出しのみ
    - _Requirements: 要件1, 要件2, 要件3, 要件4_
  
  - [x] 7.10 MusicListPageのUnit Test作成
    - 楽曲一覧ページアクセス時のテーブル表示
    - 新規登録ボタンクリック時のフォーム表示
    - 削除ボタンクリック時の確認ダイアログ表示
    - アーティスト追加完了後の自動選択
...
- [ ] 8. ルーティングとアプリケーション統合
  ...
  - [x] 8.4 グローバル通知Dialogの実装
    - [x] 8.4.1 NotificationDialog.vueのスケルトン作成
      - propsの型定義のみ（message、type: 'success' | 'error' | 'info'、visible）
      - テンプレートは空のdiv
      - _Requirements: 要件2, 要件3, 要件4, 要件6_
    - [x] 8.4.2 NotificationDialogのUnit Test作成
      - 成功メッセージ表示のテスト
      - エラーメッセージ表示のテスト
      - 情報メッセージ表示のテスト
      - 非表示時にコンテンツが表示されないテスト
      - テストは失敗する状態（Red）
      - _Requirements: 要件6_
    - [x] 8.4.3 NotificationDialog.vueの実装
      - type別のスタイル適用（success: green、error: red、info: blue）
      - 自動クローズ機能（3秒後に非表示）
      - すべてのUnit Testがパスすることを確認（Green）
      - _Requirements: 要件2, 要件3, 要件4, 要件6_
    - [x] 8.4.4 useNotificationの更新
      - 通知状態（message、type、visible）のreactive管理を追加
      - showSuccess / showError / showInfo がreactive stateを更新するよう修正
      - _Requirements: 要件2, 要件3, 要件4, 要件6_
    - [x] 8.4.5 App.vueにNotificationDialogを配置
      - useNotificationの状態をApp.vueで参照
      - NotificationDialogコンポーネントをApp.vueに追加
      - _Requirements: 要件6_
