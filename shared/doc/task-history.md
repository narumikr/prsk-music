...
**次のタスク: E2Eテストの再実行（バックエンドAPI接続確認）**

バックエンドAPIサーバーが`http://localhost:8080`で起動していることを確認し、E2Eテストを再実行してください。

```bash
# Vite開発サーバーを再起動（別ターミナル）
pnpm dev
...
- `tests/e2e/fixtures.ts` - カスタムフィクスチャ
- `tests/e2e/helpers.ts` - ヘルパー関数
- `tests/e2e/mock-data.ts` - モックデータ
- `tests/e2e/setup.ts` - グローバルセットアップ
- `tests/e2e/README.md` - E2Eテストのドキュメント
- `tests/e2e/example.spec.ts` - Playwright設定確認用のサンプルテスト
- `tests/e2e/music-management.spec.ts` - 楽曲管理機能のE2Eテスト（23件）
- `tests/e2e/artist-management.spec.ts` - アーティスト管理機能のE2Eテスト（20件）
- `tests/e2e/artist-from-music-form.spec.ts` - 楽曲フォームからのアーティスト追加E2Eテスト（3件）
- `tests/e2e/error-handling.spec.ts` - エラーハンドリングのE2Eテスト（9件）
